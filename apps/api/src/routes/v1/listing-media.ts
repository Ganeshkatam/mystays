...
export async function registerListingMediaRoutes(
  ...
  app.post<{ Params: { id: string }; Body: Body }>(
    ...
    async (request, reply) => {
      ...
      if (
        !b?.objectKey ||
        !["image", "video"].includes(b.mediaType) ||
        !Number.isInteger(b.sortOrder ?? 0) ||
        Number(b.sortOrder ?? 0) < 0
      )
        return reply.code(422).send({
          data: null,
          error: {
            code: "VALIDATION_FAILED",
            message: "Listing media input is invalid.",
          },
          meta: { requestId },
        });
      try {
        const data = await addListingMedia(
          new ListingMediaRepository(createDatabaseClient(process.env, token)),
          {
        ...
        if (e instanceof Error && e.message === "INVALID_LISTING_MEDIA")
          return reply.code(422).send({
            data: null,
            error: {
              code: "VALIDATION_FAILED",
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const data = await new ListingMediaRepository(
        createDatabaseClient(process.env, token),
      ).listByListing(request.params.id);
  ...
  app.delete<{ Params: Params }>(
    ...
    async (request, reply) => {
      ...
      const data = await new ListingMediaRepository(
        createDatabaseClient(process.env, token),
      ).delete(request.params.mediaId);
      if (!data)
        return reply.code(404).send({
          data: null,
          error: {
            code: "NOT_FOUND",
            message: "Listing media was not found.",
          },
          meta: { requestId },
        });
      return reply.send({ data, error: null, meta: { requestId } });
