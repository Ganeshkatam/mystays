...
export async function registerPropertyRoutes(
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const db = createDatabaseClient(process.env, token);
      const provider = (await new (
        await import("@mystays/database")
      ).ProviderRepository(db).getOwn(userId)) as { id: string } | null;
      const data = provider
        ? await new PropertyRepository(db).listOwn(provider.id)
        : [];
