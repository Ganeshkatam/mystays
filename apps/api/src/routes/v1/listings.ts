...
interface CreateListingBody {
  providerId: string; propertyId: string; inventoryId: string;
  title: string; description: string; monthlyRent: number; deposit: number;
}
interface ListingParams { id: string; }

export async function registerListingRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: CreateListingBody }>('/api/v1/listings', { preHandler: async (request, reply) => authenticate(request, reply) }, async (request, reply) => {
  ...
  app.post<{ Params: ListingParams }>('/api/v1/listings/:id/publish', { preHandler: async (request, reply) => authenticate(request, reply) }, async (request, reply) => {
    const requestId = String(request.id);
    const token = getBearerToken(request);
    if (!token) return reply.code(401).send({ data: null, error: { code: 'AUTHENTICATION_REQUIRED', message: 'Authentication is required.' }, meta: { requestId } });
    const db = createDatabaseClient(process.env, token);
