...
export async function registerListingRoutes(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const data = await updateListing(
          new ListingRepository(createDatabaseClient(process.env, token)),
          request.params.id,
          request.body,
        );
        ...
        if (error instanceof Error && error.message === "VALIDATION_FAILED")
          return reply.code(422).send({
            ...
            error: {
              ...
              message: "Listing update is invalid.",
            },
            meta: { requestId },
          });
        throw error;
  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      const minRent = q.minRent === undefined ? undefined : Number(q.minRent),
        maxRent = q.maxRent === undefined ? undefined : Number(q.maxRent);
      if (
        ...
        offset < 0 ||
        Number.isNaN(minRent) ||
        Number.isNaN(maxRent) ||
        (minRent !== undefined && minRent < 0) ||
        (maxRent !== undefined && maxRent < 0) ||
        (minRent !== undefined && maxRent !== undefined && minRent > maxRent)
      )
        return reply.code(422).send({
          data: null,
          error: {
      ...
      const data = await import("@mystays/application").then(
        ({ searchListings }) =>
          searchListings(new ListingRepository(db), {
            city: q.city?.trim() || undefined,
            propertyType: q.propertyType?.trim() || undefined,
            inventoryType: q.inventoryType?.trim() || undefined,
            minRent,
            maxRent,
            limit,
            offset,
  ...
  app.post<{ Body: CreateListingBody }>(
    ...
    async (request, reply) => {
      ...
      if (validateListingInput(request.body).length > 0)
        return reply.code(422).send({
          data: null,
          error: {
            code: "VALIDATION_FAILED",
  ...
  app.post<{ Params: ListingParams }>(
    ...
    async (request, reply) => {
      ...
      const { data: listing, error } = await db
        .from("listings")
        .select("id,status,title,description,monthly_rent,deposit")
      ...
      const data = await publishListing(
        new ListingRepository(db),
        {
          ...
          monthlyRent: Number(listing.monthly_rent),
