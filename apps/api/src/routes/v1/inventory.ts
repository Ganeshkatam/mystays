...
export async function registerInventoryRoutes(
  ...
  app.patch<{
    Params: { id: string };
    Body: { status: "available" | "occupied" | "inactive" };
  }>(
    ...
    async (request, reply) => {
      ...
      if (!["available", "occupied", "inactive"].includes(status ?? ""))
        return reply.code(422).send({
          data: null,
          error: {
            code: "VALIDATION_FAILED",
            message: "Inventory status is invalid.",
          },
          meta: { requestId },
        });
      try {
        ...
        const data = await updateInventoryStatus(
          new InventoryRepository(createDatabaseClient(process.env, token)),
          request.params.id,
          status,
        );
        return reply.send({ data, error: null, meta: { requestId } });
      } catch (e) {
        if (e instanceof Error && e.message === "INVALID_INVENTORY_STATUS")
          return reply.code(422).send({
            data: null,
            error: {
              code: "VALIDATION_FAILED",
  ...
  app.post<{ Body: Body }>(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        !b?.inventoryType ||
        !b?.label ||
        !Number.isInteger(b.occupancyCapacity) ||
        b.occupancyCapacity < 1 ||
        b.occupancyCapacity > 100
      )
        return reply.code(422).send({
          data: null,
          error: {
  ...
  app.get<{ Querystring: { propertyId: string } }>(
    ...
    async (request, reply) => {
      const requestId = String(request.id),
        token = getBearerToken(request);
      if (!token || !request.query.propertyId)
        return reply.code(422).send({
          data: null,
          error: {
            code: "VALIDATION_FAILED",
      ...
      const db = createDatabaseClient(process.env, token);
      const data = await new InventoryRepository(db).listOwn(
